...
async fn main() -> Result<()> {
    ...
    info!(
        "📡 Service: {} | Version: {} (Smart Alpha Update)",
        env!("CARGO_PKG_NAME"),
                ...
                if window.buckets.len() >= 25 {
                    ...
                    if total_ticks >= min_ticks {
                        ...
                        let sentiment = if sent_ts == 0 {
                            ...
                            let td = trade.timestamp - sent_ts;
                            if td < news_decay_ms && td >= 0 {
                                sent * (1.0 - (td as f64 / news_decay_ms as f64))
                            } else {
                                0.0
                            }
                        };
                            ...
                            tokio::spawn(async move {
                                ...
                                .await
                                {
                                    if let Some(best) = res.result.iter().find(|r| {
                                        ...
                                        // 🔥 CERRAHİ 2: SİNYAL DOĞRULAMASI (Momentum Ignition)
                                        // Sadece hız (Z-Vel) yeterli değil, tahta (Z-Imb) da onu desteklemeli.
                                        let mut s_type = SignalType::Hold;

                                        if z_vel > vel_buy_thresh && z_imb > 0.5 {
                                            s_type = SignalType::Buy; // Yükseliyor ve Emir defteri ALIM dolu.
                                        } else if z_vel < vel_sell_thresh && z_imb < -0.5 {
                                            s_type = SignalType::Sell; // Düşüyor ve Emir defteri SATIŞ dolu.
                                        }

                                        if s_type != SignalType::Hold {
                                            let signal = TradeSignal {
                                                symbol: sym_clone.clone(),
                                                r#type: s_type as i32,
